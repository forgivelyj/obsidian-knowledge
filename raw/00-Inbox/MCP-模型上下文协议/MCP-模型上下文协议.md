...
![mermaid\-diagram\-2026\-04\-27\-140503\.png](图片和附件/mermaid-diagram-2026-04-27-140503.png)
...
## 2\. HTTP with SSE \(Server\-Sent Events\)（官方已弃用）
...
- **工作原理**：
    ...
    1. **建立会话 \(GET /sse\)**：
        ...
        - 客户端发起 GET 请求连接 SSE 端点。

        - 服务器返回一个唯一的 **Session ID**。

        - 这条连接保持打开，服务器通过它推送（Push）消息给客户端。

    2. **发送指令 \(POST /messages\)**：

        - 客户端通过独立的 POST 请求发送 JSON\-RPC 消息。

        - **关键点**：必须在 URL 参数或 Body 中带上刚才的 **Session ID**，否则服务器不知道你是谁。
...
[first\_server\_sse\.py](图片和附件/first_server_sse.py)

[first\_client\_sse\.py](图片和附件/first_client_sse.py)
...
[first\_client\_http\.py](图片和附件/first_client_http.py)

[first\_server\_http\.py](图片和附件/first_server_http.py)

# langchain中接入MCPServer
