...
![095\.png](图片和附件/095.png)

#### 3\. RAG系统工作流程图解

- RAG'圣经'论文：[https://arxiv\.org/pdf/2312\.10997](https://arxiv.org/pdf/2312.10997)

![004\.png](图片和附件/004.png)
...
![005\.png](图片和附件/005.png)

### 二\. Naive RAG

Naive RAG遵循一个传统的处理流程，包括索引、检索和生成三个阶段

![006\.png](图片和附件/006.png)

#### 1\. 文档分块

![007\.png](图片和附件/007.png)
...
4. **One\-hot编码\(词汇表\)**:在机器学习和深度学习的应用中，文本数据通常需要转换为数值型数据，One\-hot编码是一种常用的转换方式。在One\-hot编码中，每个汉字都被表示为一个只有一个元素为1其余元素全为0的向量。向量的长度等于汉字的总数量，1的位置表示该汉字的索引。One\-hot编码的一个主要优点是它简单直观，但当汉字的数量非常大时，One\-hot编码会占用大量的内存且无法表达语义关系 \.
...
![086\.png](图片和附件/086.png)

- 欧式距离L2：通过计算向量之间的欧几里得距离来衡量相似度。

- 点积：计算两个向量的点积，适合归一化后的向量。

![087\.png](图片和附件/087.png)



![016\.png](图片和附件/016.png)
...
print("当前向量距离算法:", collection.configuration["hnsw"]["space"])
