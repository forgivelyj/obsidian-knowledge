...
# RFC 8707: Resource Indicators for OAuth 2.0 摘要
...
## 核心价值与安全作用
- **[[Audience-Restriction]]（受众限制）**：AS 可以通过 JWT 中的 `aud` 声明，对颁发的 Access Token 进行受众隔离。防止受保护资源（RS1）拿着客户端发来的 Token 去非法访问另一个资源（RS2）（即防止 Token 重定向攻击）。
